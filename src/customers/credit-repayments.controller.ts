...
export class CreditRepaymentsController {
  ...
  async cash(@Param('customerId') customerId: string, @Req() request: AuthenticatedRequest, @Body() data: CashCreditRepaymentDto) { this.logger.log(`Cash repayment request for customer ${customerId}`); return { status: 201, data: await this.service.cash(request.user!.companyId, customerId, request.user!.id, data), error: null }; }
  @Post('credit-repayments/mpesa') @Version('1') @RequireShopAccess('shopId')
  async mpesa(@Param('customerId') customerId: string, @Req() request: AuthenticatedRequest, @Body() data: MpesaCreditRepaymentDto) { this.logger.log(`M-Pesa repayment request for customer ${customerId}`); return { status: 201, data: await this.service.mpesa(request.user!.companyId, customerId, request.user!.id, data), error: null }; }
  @Patch('credit-repayments/:repaymentId/verify') @Version('1') @RequireShopAccess('shopId', 'OWNER', 'MANAGER')
  async verify(@Param('customerId') customerId: string, @Param('repaymentId') repaymentId: string, @Req() request: AuthenticatedRequest, @Body() data: VerifyCreditRepaymentDto) { return { status: 200, data: await this.service.verify(request.user!.companyId, customerId, repaymentId, request.user!.id, data), error: null }; }
}
