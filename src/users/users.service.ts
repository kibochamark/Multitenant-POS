...
export class UsersService {
  ...
  async getCurrentUserContext(userId: string) {
    return this.userrepo.getCurrentUserContext(userId);
  }
  ...
  async findusersbycompanyid(companyId: string, skip?: number, take?: number) {
    try {
      ...
      const users = await this.userrepo.findUsersByCompanyId(
        companyId,
        skip,
        take,
      );
  ...
  async findusershoprolebycompositeid(userId: string, shopId: string) {
    try {
      this.logger.log(
        'calling user repo to find user shop role by composite id',
      );

      const usershoprole = await this.userrepo.findUserShopRoleByCompositeId(
        userId,
        shopId,
      );
  ...
  async updateusershoprolebycompositeid(
    ...
    try {
      this.logger.log(
        'calling user repo to update user shop role by composite id',
      );

      const usershoprole = await this.userrepo.updateUserShopRoleByCompositeId(
        userId,
        shopId,
        data,
      );
  ...
  async deleteusershoprole(userId: string, shopId: string) {
    try {
      ...
      const usershoprole = await this.userrepo.deleteUserShopRole(
        userId,
        shopId,
      );
