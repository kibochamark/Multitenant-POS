...
export class CompanyController {
  ...
  async createShop(
    ...
    @Body() shopData: CompanyDTO,
  ) {
    try {
      const createdShop = await this.companyService.createShop(
        shopData,
        parentCompanyId,
      );
  ...
  @Patch('shop/:id')
  @Version('1')
  @RequireShopAccess('id', 'OWNER', 'MANAGER')
  async updateShop(
    @Param('id') id: string,
    @Body() shopData: Partial<CompanyDTO>,
  ) {
    try {
  ...
  @Version('1')
  async findShopsByCompanyId(
    @Param('companyId') companyId: string,
    @Param('skip') skip?: number,
    @Param('take') take?: number,
  ) {
    try {
      const shops = await this.companyService.findShopsByCompanyId(
        companyId,
        skip,
        take,
      );
